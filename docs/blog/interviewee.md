...
// 请从一个字符串中找出一个最长的不包含重复字符的子字符串，计算该最长子字符串的长度
...
1. 它用一组连续的内存空间。是线性表。
2. 连续的内存空间，比如要想在数组中插入、删除一个数据，为了保证连续性，就需要做大量的数据搬移工作。
3. 计算机会给每个内存单元分配一个地址，计算机通过地址来访问内存中的数据。

链表适合插入、删除，时间复杂度为O(1)。数组适合随机访问，根据下标随机访问的时间复杂度为O(1)。
...
1. 物理层、数据链路层、网络层、数据传输层、会话层、表示层、应用层
...
1. Move Permanently。永久重定向，该操作比较危险，需要谨慎操作，过一段时间后想取消，但是浏览器会有缓存

## 302

1. Fount。临时重定向，重定向时method -> post会变成get。

## 307

1. Temporary Redirect。临时重定向，在重定向时不会改变method
...
1. 网络进程会查找本地缓存是否缓存了该资源，如果有缓存资源，那么直接返回资源给浏览器进程
2. DNS域名解析
3. TLS握手
4. TCP握手
5. 网络进程发起网络请求，服务器返回HTTP数据到网络进程，网络进程解析HTTP数据，转发给浏览器进程
6. 浏览器进程收到网络进程传过来的HTTP数据，发送CommitNavigation消息到渲染进程。携带response HTTP数据
7. 渲染进程和浏览器进程建立数据管道接收HTML数据
8. 渲染
   1. 将html数据转化为DOM树
      1. script link的位置
   2. 将css样式表转化为浏览器可以理解的styleSheets，计算出DOM节点的样式
   3. 创建布局树，计算元素的布局信息
   4. 对布局树进行分层，生成分层树（z-index）
   5. 为每个图层生成绘制列表，将其提交给合成线程
